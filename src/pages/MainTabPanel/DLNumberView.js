...
class DLNumberView extends React.Component {


  _titles = ["DL Number", "DL State" ]
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      dl: initDLNumber,
      dlState: (props.initDLState != null && props.initDLState != '') ? props.initDLState : SharedUtility.defaultState,
      
      isShowDLStatePickerModal: false,
...
// // GetRequestByDL(string OLN, string OLS, string TYP, string ORI="NJ")
// // GetRequestByDLFull(string OLN, string OLS, string ORI="NJ")


//       let params = 'LIC=' + plate + '&LIS=' + state + '&LIY=2018' + '&TYP=' + Type
...
//   };
  ...
  onDL = async (typeid, plate, state)=>{
    let reqId = await this.getDLRun(typeid, plate, state);
    let result = await this.getRunResult(reqId);
    return result
  }

  getDLRun = async ( Type, OLN, state)=>{
			let RequestID = null;
			try {
				console.log ("Starting getDLRun");

// // GetRequestByDL(string OLN, string OLS, string TYP, string ORI="NJ")
// // GetRequestByDLFull(string OLN, string OLS, string ORI="NJ")

        let params = "OLN=" + OLN + "&OLS=" + state
        ...
        if(rem != -1 && rem != null){
          RequestID = rem.RequestID;
				...
				console.log ("LINE 606 >  getDLRun RequestID is " + RequestID);
			} catch (e) {
				console.log ("LINE 608 > getDLRun >> " + e.message);
      }
  ...
  actionFull = async ()=>{
    ...
    if (!this.isValidAllFields ()) {
      ...
      let result = await this.onDL(2, this.state.dl, this.state.dlState);
      showPageLoader(false);
  ...
  isValidAllFields = () => {
    ...
    if (!this.state.dl) {
      Alert.alert('Validation Error', 'Please input DLNumber');
      return false;
    }

    if (!this.state.dlState) {
      Alert.alert('Validation Error', 'Please select DL State');
      return false;
    }
  ...
  onTapRandom = async () => {
    ...
    let result = await this.onDL(1, this.state.dl, this.state.dlState);
    showPageLoader(false);
  ...
  clearRequestFields() {
    ...
    this.setState({
      dl: null,
      dlState: SharedUtility.defaultState,
      plateType: null,    
    ...
    return (
      <ScrollView
        ...
        <View style={styles.container}>
          ...
          <TextInput
            ...
            onChangeText={(text) => {
              ...
              this.setState({dl: text.toUpperCase()});
              
            }}
            placeholder={''}
            onSubmitEditing={() => {
              this.setState({isShowDLStatePickerModal: true});
            }}
          />
        ...
        <PickerModal
          ...
          onTapClose={() => {
            this.setState({isShowDLStatePickerModal: false});
          }}
          isShow={this.state.isShowDLStatePickerModal}
        />
...
export default function (props) {
  ...
  return <DLNumberView {...props} navigation={navigation} route={route} />;
}
