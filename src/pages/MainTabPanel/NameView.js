...
class NameView extends React.Component {
  ...
  getPlateRun = async (Type, plate, state) => {
    try {
      let RequestURL = this.CreateRequest(
        'LIC=' + plate + '&LIS=' + state + '&LIY=2018' + '&TYP=' + Type,
      );

      console.log('Starting getPlateRun : ' + RequestURL);

      if (!!this.state.plateType) {
        RequestURL = RequestURL + '&LIT=' + this.state.plateType;
      }
      ...
      console.log(
        'LINE 407 >> BACKGROUND_PROC getPlateRun error : > ',
        e.message,
      );
      return null;
    }
  ...
  onNameRun = async (typeid,  fname,  lname, mname, dob,  state, gender)=>{
    let reqId = await this.getNameRun(2, fname, lname, mname, dob, state, gender);
    let res = await this.getRunResult(reqId);
    return res;
  }

  getNameRun = async (Type, First, Last, MI, DOB, state, Gender)=>{
			let RequestID = null;
			try {

        let params = "NMF=" + First + "&NML=" + Last + "&DOB=" + DOB + "&TYP=" + Type 

        if(!Utils.isNullOrEmptyStr(MI)){
          params += "&MIN=" + MI
        }
        ...
        if(!Utils.isNullOrEmptyStr(Gender)){
          params += "&SEX=" + Gender
        }
        ...
        if(rem != null && rem != -1){
          RequestID = rem.RequestID;
				...
				console.log("getNameRun RequestID is " + RequestID);
			} catch (e) {
  ...
  actionFull = async ()=>{
    ...
    showPageLoader(true)
    //Task.Run (async () => {
    let result = await this.onNameRun (2, this.state.firstName, this.state.lastName, this.state.middleName, this.state.dob, this.state.plateState, this.state.sex );
  ...
  isValidAllFields = () => {
    ...
    //   Alert.alert('Validation Error', 'Please input middle name');
    //   return false;
    // }
    ...
    //   Alert.alert('Validation Error', 'Please select suffix');
    //   return false;
    // }

    // if (!this.state.dob) {
    //   Alert.alert('Validation Error', 'Please select DOB');
    //   return false;
    // }
    // if (!this.state.sex) {
    //   Alert.alert('Validation Error', 'Please select Sex');
    //   return false;
    // }
    ...
    const { firstName, lastName, dob, sex, plateState } = this.state;
    if ( !firstName || !lastName || !dob || !sex || !plateState ) {
  ...
  changePlateState = (str) => {
    if (str == null) {
      str = SharedUtility.StateList[0];
    }

    // if(Utils.isNullOrEmptyStr(this.state.suffix)){
    //   Alert.alert('Validation', 'Please select suffix first.', [
    //     {
    //       text:'Ok',
    //       style:'default',
    //       onPress:()=>{
    //         this.setState({isShowSuffixPicker: true})
    //       }
    //     }
    //   ])
    ...
    if (str == 'NJ' || str == 'NY') {
      this.state.suffix && this.setState({suffix: this.state.suffix.substring(0, 1)});
    }

    if (str != SharedUtility.defaultState) {
      // this.addPlateTypeInput ();
      this.setState({
      ...
      // this.changeButtonLocation(
      //   BaseVC.curSelIndex != 1,
      //   BaseVC.curSelIndex == 1,
      // );
      this.changeButtonLocation(
        true,
      ...
      <ScrollView
        ...
        <View style={styles.container}>
          ...
          <TextInput
            ...
            onChangeText={(text) => {
              this.setState({firstName: text.toUpperCase()});
            }}
            placeholder={''}
          ...
          <TextInput
            ...
            autoCapitalize="characters"
            onChangeText={(text) => {
              this.setState({middleName: text.toUpperCase()});
            }}
            placeholder={''}
          ...
          <TextInput
            ...
            autoCapitalize="characters"
            onChangeText={(text) => {
              this.setState({lastName: text.toUpperCase()});
            }}
            placeholder={''}
            onSubmitEditing={() => {this.setState({isShowSuffixPicker:true})}}
          />
          ...
          <View style={styles.dobContainer}>
            <MaskInput
              ...
              onChangeText={(masked, unmasked, obfuscated) => {
                this.setState({dob: masked});
              }}
              keyboardType={"number-pad"}
              mask={Masks.DATE_MMDDYYYY}
              onEndEditing={(e) => {
                if (this.state.dob.length > 0 && this.state.dob.length < 10) {
          ...
          <TouchableOpacity
            onPress={() => {
              this.setState({isShowSexPicker: true});
            }}>
            <TextInput
              style={styles.InputText}
        ...
        <PickerModal
          pickList={SharedUtility.SuffixList}
          selectedIndex={0}
          onTapSelect={(index, val) => {
            let str = val;
            if (str == null) {
              str = SharedUtility.SuffixList[0];
            }

            if (str == 'Other') {
              // Global.ShowAlertTextTitle ("", "Please input the other suffix.", "Suffix", "", Global.kAppDelegate.Window.RootViewController, (obj) => {
              //   // OK block
              //   tf.Text = obj.Text;
              // }, (obj) => { });
              ...
              if (this.state.plateState == 'NJ' || this.state.plateState == 'NY') {
                this.setState({suffix: str.substring(0, 1), isShowSuffixPicker: false});
              } else {
                this.setState({suffix: str, isShowSuffixPicker: false});
              }
            }
          }}
          onTapClose={() => {
            this.setState({isShowSuffixPicker: false});
          }}
          isShow={this.state.isShowSuffixPicker}
        />
        ...
        <PickerModal
          ...
          onTapClose={() => {
            this.setState({isShowSexPicker: false});
          }}
          isShow={this.state.isShowSexPicker}
        />

        <InputModal
          title={'Please input the other suffix.'}
          onTapClose={() => {
            this.setState({isShowInputModal: false});
          }}
          onTapOk={(val) => {
            if (this.state.plateState == 'NJ' || this.state.plateState == 'NY') {
              this.setState({suffix: val.substring(0, 1), isShowInputModal: false});
            } else {
              this.setState({suffix: val, isShowInputModal: false});
            }
          }}
          defVal={''}
...
export default function (props) {
  ...
  return <NameView {...props} navigation={navigation} route={route} />;
}
