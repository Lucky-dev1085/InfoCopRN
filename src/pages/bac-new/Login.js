...
// import { PlateRecognizer } from './MainTabPanel/PlateRecognizer';
 ...
 async componentDidMount() {
  ...
  async checkUserData(user_id) {
    ...
    if(this.MSG.toLowerCase() == 'invalid user id'){
      Alert.alert('Login Error', 'Invalid User ID. Please try again with correct User ID.')
  ...
  async SetHighRisk() {
    ...
    try {
      ...
      let RequestURL = 'https://' + serverAddr + '/api/v1beta/login?';
  ...
  async returnonui() {
    ...
    if (this.LogIn) {
      // alert('abcd')
      SharedUtility.saveString('am', JSON.stringify({
        'pin': this.state.pin,
        'pwd': this.state.pwd,
  ...
  async getLogIn() {
    try {
      ...
      RequestURL = 'https://' + server + '/api/v1beta/login?';
      RequestURL += 'DIN=' + DIN + '&';
      RequestURL += 'DCN=' + DCN + '&';
