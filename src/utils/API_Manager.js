...
export default class API_Manager {
    ...
    static apiUrl = "https://demo.infocopmobile.com/api/v1beta/";

    static CheckUserIDurl = "login?STP=User&UID=";


    getAPIKey = async (urlToRead, un, pw, ver,  del) => { //Task<ServiceResponse>
    ...
    getRootUrl = ()=>{
        ...
        let value = "";
        if (sm != null) {
            value = `https://${sm.serverAddress}/api/v1beta/`;

        } else {
            value = "https://demo.infocopmobile.com/api/v1beta/";
        }
        return value;
    ...
    CheckUserID = (userId) =>{
        ...
        let url = API_Manager.apiUrl + "" +  API_Manager.CheckUserIDurl + "" + userId

        return new Promise( (resolve, reject)=>{
            requestCall( url, 'get', null, null, (res, err)=>{
                if(err){
                    reject( err ) 
                }else{
    ...
    ProcessLogOff = async()=> {
        ...
        let LogInRsp = await this.MakeRequest ("https://" + serverip+ "/api/v1beta/logoff", apikey, null);
    ...
    CheckLogOff = async ()=> {
        ...
        let sr =  await this.MakeRequest ("https://" + serverip + "/api/v1beta/logoff?onoff=doit", apikey, null);
    ...
    MakeRequestPlate = async( urlToRead, apikey, stuff)=>{ // string urlToRead, string apikey, string[] stuff

        console.log(' LINE 305 >> MakeRequestPlate >>  urlToRead, apikey, stuff >> : ', urlToRead, apikey, stuff)
        ...
        console.log("getMakeRequestResonse  response >>> ", JSON.stringify(res, null, 4))
    ...
    UpdateNotes = async (params, CompletionHandler) => {
        let apikey = (await SharedUtility.GetSharedAPIKey()).getAPIKey ("NULL");
        let serverip = SharedUtility.sharedSettingModel.getServerAddr ("NULL");
        let _url = "https://" + serverip + "/api/v1beta/icrequest?" 
        let pms = 'CSN=' + params.CSN + '&NTS=' + params.NTS + '&Dept=' + params.RIN;

        let sr = await this.MakePostRequest(_url + pms + "&MID=" + apikey, apikey, null);
    ...
    GetUserStatus = async (CompletionHandler) => {
        ...
        let serverip = SharedUtility.sharedSettingModel.getServerAddr ("NULL");
        // alert(serverip)
        let sr =  await this.MakePostRequest ("https://" + serverip + "/api/v1beta/icrequest?MID=" + apikey, apikey, null);
    ...
    UpdateUserStatus = async (sta, zone, staff, CompletionHandler) => {
        ...
        let sr =  await this.MakePostRequest ("https://" + serverip + "/api/v1beta/icrequest?STA=" + sta + "&LOC=" + zone + "&GPS=GPSCOOR&ADR=" + staff + "&MID=" + apikey, apikey, null);
    ...
    GetHazmat = async (hazmat, CompletionHandler) => {
        ...
        let serverip = SharedUtility.sharedSettingModel.getServerAddr ("NULL");
        // alert(serverip)
        let sr =  await this.MakePostRequest ("https://" + serverip + "/api/v1beta/icrequest?HAZMAT=" + hazmat, apikey, null);
    ...
    GetHistory = async (CompletionHandler) => {
        ...
        let serverip = SharedUtility.sharedSettingModel.getServerAddr ("NULL");
        // alert(serverip)
        let sr =  await this.MakePostRequest ("https://" + serverip + "/api/v1beta/icrequest?HIST=50", apikey, null);
    ...
    ResetPasswordApi = async (params, CompletionHandler) => {
      ...
      + '&OTPSentTime=' + params.OTPSentTime
      + '&OTP=' + params.OTP
    ...
    ResetPasswordApi6 = async (params, CompletionHandler) => {
        ...
        if(params.ChangeOtherSettings)
        {
            ...
            + '&NewPassword=' + params.NewPassword
            + '&ConfirmPassword=' + params.ConfirmPassword
            + '&ChangeOtherSettings=' + params.ChangeOtherSettings
            ...
            + '&SecurityPhoto=' + params.SecurityPhoto
        } else {
            pms = 'EmpID=' + params.EmpID
                    + '&DeviceCodeName=' + params.DeviceCodeName
                    + '&Dept=' + params.Dept
    ...
    CheckUpdateApp = async (CompletionHandler) => {
        if (SharedUtility.sharedSettingModel === null) {
            ...
            let sr =  await this.MakeRequest (
                "https://" + serverip + "/api/v1beta/icrequest?GetUpdate=" + VersionNumber.buildVersion, 
                apikey, 
                null
            );
            CompletionHandler(sr);
